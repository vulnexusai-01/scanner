...
Classic mistake: hiding sensitive content in `robots.txt`. It is a public file — anyone can read it and access the listed URLs directly. To protect for real, block access at the server level (auth, access rules) and treat `robots.txt` only as an indexing hint.
